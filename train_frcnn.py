...
from PIL import Image  # python  图像处理模块
import torchvision
from torchvision.models.detection import MaskRCNN
from torchvision.models.detection.rpn import AnchorGenerator  # 生成预选框
...
def MobileNetV2(num_classes, out_channel):
    # load an instance segmentation model pre-trained on COCO
    backbone = torchvision.models.mobilenet_v2(pretrained=True).features
    # output channels in a backbone. For mobilenet_v2, it's 1280
    backbone.out_channels = out_channel

    # make the RPN generate 5 x 3 anchors per spatial location
    anchor_generator = AnchorGenerator(sizes=((32, 64, 128, 256, 512),),
                                       aspect_ratios=((0.5, 1.0, 2.0),))
    # define what are the feature maps
    roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'],
                                                    output_size=7,
                                                    sampling_ratio=2)
    # 1280 is out of range,so we need to reduce it
    backbone.add_module("reduce_con", torch.nn.Conv2d(1280, out_channel, kernel_size=(1, 1), stride=(1, 1), bias=False))
...
#     print("use " + str(torch.cuda.device_count()), 'to operate')
#     pass
